...
Scenario: Import into a non-empty class
    ...
    And "Class A" already has the student with ID "111"
    And the system has a student with ID "222" who is not in "Class A"
    ...
    And the "Class A" student list now contains both students "111" and "222"
...
Scenario: Partial import: blank registration
    ...
    And "Class A" now has student "111" enrolled

Scenario: Import would exceed the class capacity
    Given I am logged in as a teacher and on the "Class A" page
    And "Class A" has a limit of 50 seats and already has 48 enrolled students
    ...
    And the "Class A" student list remains with 48 students only
